class Car:
    ...
    def stop(self):
        print(f"{self.name}: Машина отснаовилась")

    def turn(self, direction):
        print(f"{self.name}: Машина повернула {'налево' if direction == 0 else 'направо'}")

    def show_speed(self):
        pront(f"{self.name}: Скорость автомобиля - {self.speed}")


class WorkCar(Car):
    def show_speed(self):
        return f"{slef.name}: Скорость автомобиля - {self.speed} - ПРЕВЫШЕНИЕ СКОРОСТИ!!!" \
